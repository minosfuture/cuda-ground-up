...
__global__ void kernel_gemm_4(half *A, half *B, half *C, int M, int N, int K,
  ...
  for (int tile_idx = 0; tile_idx < K / kTileK; tile_idx++) {
    ...
    for (int tile_y_idx = 0; tile_y_idx < kTileYDim; tile_y_idx++) {
      A_shmem[(start_y_idx + tile_y_idx) * kTileK + threadIdx.x] =
          A[(start_y_idx + tile_y_idx) * K + threadIdx.x];
      B_shmem[(threadIdx.y * kTileYDim + tile_y_idx) * blockDim.x +
              threadIdx.x] = B[(start_y_idx + tile_y_idx) * N + threadIdx.x];
    }
    __syncthreads();
    A += kTileK;
    B += kTileK * N;
  ...
  for (int tile_y_idx = 0; tile_y_idx < kTileYDim; tile_y_idx++) {
    half &C_elem = C[(start_y_idx + tile_y_idx) * N + col];
